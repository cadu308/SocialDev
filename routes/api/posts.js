...
router.post(
    ...
    [ auth,
      [ 
          check('texto', 'É necessário informar o texto.').not().isEmpty() 
      ]  
    ], 
    async (req, res) => {
...
router.put('/like/:id', auth, async (req, res) => {
    try {
        ...
        if (post.likes.filter(like => like.user.toString() === req.user.id).length > 0) {
            return res.status(400).json({ msg: 'Não é permitido dar mais de um like no post.' });
        }
...
router.put('/dislike/:id', auth, async (req, res) => {
    try {
        ...
        //Checar se o usuário já deu o like
        if (post.likes.filter(like => like.user.toString() === req.user.id).length === 0) {
            return res.status(400).json({ msg: 'Você ainda não deu like neste post.' });
        }
